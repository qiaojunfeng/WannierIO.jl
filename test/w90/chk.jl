@testitem "read chk" begin
    ...
    @test chk.M[:, :, 2, 1] ≈ ref_M12

    @test size(chk.Uml) == (4, 4, 216)
    ref_U2 = ComplexF64[
    ...
    @test size(chk.Udis) == (0, 0, 0)
    @test chk.checkpoint == "postwann"
    @test size(chk.dis_bands) == (0, 0)
...
@testitem "read chk disentanglement" begin
    ...
    @test chk.n_kpts == 9^3
    @test chk.n_bvecs == 8
    @test size(chk.M) == (8, 8, 8, 9^3)
    @test size(chk.dis_bands) == (16, 9^3)
    @test all(chk.dis_bands)
...
@testitem "read/write chk lattice" begin
    ...
    @test all(isapprox.(chk.lattice, wout["lattice"]; atol = 3.0e-7))
    @test all(isapprox.(chk.recip_lattice, wout["recip_lattice"]; atol = 3.0e-7))
