...
function _read_unk_fmt(filename::AbstractString)
    ns = occursin("NC", filename) ? 2 : 1
    io = open(filename)

    line = split(strip(readline(io)))
    n_gx, n_gy, n_gz, ik, n_bands = parse.(Int, line)

    Ψ = zeros(ComplexF64, n_gx, n_gy, n_gz, n_bands, ns)
...
function read_unk(filename::AbstractString)
    ...
    n_gx, n_gy, n_gz, n_bands, ns = size(Ψ)
    println("  n_gx    = ", n_gx)
    println("  n_gy    = ", n_gy)
    println("  n_gz    = ", n_gz)
    println("  ik      = ", ik)
...
"""
function write_unk(
    filename::AbstractString, ik::Integer, Ψ::Array; binary::Bool=false
)
    if binary
