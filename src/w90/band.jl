...
- `eigenvalues`: `n_bands × n_kpts` matrix of band energies
...
function read_w90_band_dat(io::IO)
    ...
    eigenvalues = permutedims(reshape(dat[:, 2], n_kpts, :))
    ...
    if size(dat, 2) == 3
        extras = permutedims(reshape(dat[:, 3], n_kpts, :))
...
function write_w90_band_dat(
        ...
        extras::Union{AbstractMatrix{<:Real}, Nothing} = nothing,
    )
    n_bands, n_kpts = size(eigenvalues)
    n_kpts > 0 || throw(ArgumentError("eigenvalues is empty"))
    length(x) == n_kpts ||
        throw(DimensionMismatch("x must have one entry per column of eigenvalues"))
    isnothing(extras) || size(extras) == size(eigenvalues) ||
        throw(DimensionMismatch("extras must have the same size as eigenvalues"))

    for ib in 1:n_bands
        for ik in 1:n_kpts
            if isnothing(extras)
                @printf(io, " %15.8E %15.8E\n", x[ik], eigenvalues[ib, ik])
            else
                @printf(
                    io,
                    " %15.8E %15.8E %15.8E\n",
...
`eigenvalues` is an `n_bands × n_kpts` matrix of band energies.
"""
function write_w90_band(
        prefix::AbstractString, kpath::CrystalBase.KPath, eigenvalues::AbstractMatrix{<:Real}
    )
